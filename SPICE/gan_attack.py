...
#         prob2 = F.softmax(output, dim=1)
    ...
    def train_batch(self, p, labels):
        ...
        for i in range(1):
            ...
            C = 0.2
            loss_perturb = torch.mean(torch.norm(torch.clamp(perturbation, -0.15, 0.15).view(perturbation.shape[0], -1), 2, dim=1))
#             loss_perturb = torch.max(loss_perturb - C, torch.zeros(1, device=self.device))
        ...
        return loss_D_GAN.item(), loss_G.item(), loss_perturb.item(), loss_adv.item(), loss_norm.item()
