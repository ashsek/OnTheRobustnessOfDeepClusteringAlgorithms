...
class GAN_Attack:
    ...
    def _loss(self, X, adv_X):
        ...
        alogit1 = self.net(adv_X)
        alogit2 = self.net2(adv_X)
        log_a = alogit1 + alogit2
...
#         prob2 = F.softmax(output, dim=1)
    ...
    def train_batch(self, p, labels):
        ...
        for i in range(1):
            ...
            adv_images = torch.clamp(perturbation, -0.25, 0.25) + x
        ...
        for i in range(1):
            ...
            C = 0.2
            p2 = torch.clamp(perturbation, -0.2, 0.2)
            loss_perturb = torch.mean(torch.norm(p2.view(p2.shape[0], -1), 2, dim=1))
#             loss_perturb = torch.mean(torch.norm(perturbation.view(perturbation.shape[0], -1), 2, dim=1))
#             loss_perturb = torch.max(loss_perturb - C, torch.zeros(1, device=self.device))
            
#             loss_norm = torch.mean(torch.norm(perturbation.view(perturbation.shape[0], -1), 2, dim=1))
        ...
        return loss_D_GAN.item(), loss_G.item(), loss_perturb.item(), loss_adv.item(), loss_perturb.item()
    ...
    def train(self, train_dataloader, epochs):
        for epoch in range(1, epochs+1):
            ...
            for batch_idx, (inputs, _, _, targets, indexes) in enumerate(train_dataloader):
                torch.cuda.empty_cache()
                inputs, targets = inputs.to(self.device), targets.to(self.device)

                loss_D_batch, loss_G_fake_batch, loss_perturb_batch, loss_adv_batch, loss_norm_batch = self.train_batch(inputs, targets)
                loss_D_sum += loss_D_batch
                loss_G_fake_sum += loss_G_fake_batch
                loss_perturb_sum += loss_perturb_batch
                loss_adv_sum += loss_adv_batch
                loss_norm += loss_norm_batch

            # print statistics
            self.batch_no += 1
            num_batch = len(train_dataloader)
            print("epoch %d:\nloss_D: %.3f, loss_G_fake: %.3f,\
             \nloss_perturb: %.3f, loss_adv: %.3f, loss_norm: %.3f\n " %
                  (epoch, loss_D_sum/num_batch, loss_G_fake_sum/num_batch,
                   loss_perturb_sum/num_batch, loss_adv_sum/num_batch, loss_norm/num_batch))
