...
#         with torch.no_grad():
        f = self.model(X)
        v = self.model_ema(X)
        _, g = self.model(X, True)
#         print(self.model.eval())
#         print(self.model_ema.eval())
        loss, out, posterior, pi_logit = self.elbo(f, v, g)
#         print(posterior)
#         print('pilogit ',pi_logit)
            
#         with torch.no_grad():
        f = self.model(adv_X)
        v = self.model_ema(adv_X)
        _, g = self.model(adv_X, True)
        loss, out, posterior_adv, pi_logit_adv = self.elbo(f, v, g)
#         print(posterior_adv)
...
#             diff_vec = posterior_adv[i] - posterior[i] #
...
#         return dist_loss # is beta * ||f(x) - Cluster||^2_2
        for i in range(len(pi_logit)):
            diff_vec = pi_logit_adv[i] - pi_logit[i] #
            sample_dist_loss = torch.matmul(diff_vec.view(1, -1),
                                            diff_vec.view(-1, 1))
            dist_loss += 0.5 * torch.squeeze(sample_dist_loss)
...
#         x = p
        ...
        for i in range(1):
            ...
            adv_images = torch.clamp(perturbation, -0.05, 0.05) + x
#             adv_images = torch.clamp(adv_images, self.box_min, self.box_max)
        ...
        for i in range(1):
            ...
            C = 0.1
            p2 = torch.clamp(perturbation, -0.05, 0.05)
