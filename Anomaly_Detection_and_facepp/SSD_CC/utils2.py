...
def get_features(model, dataloader, max_images=10 ** 10, verbose=False):
    ...
    for batch_idx, (inputs1, targets, index) in enumerate(dataloader):
        img = inputs1
        label = targets
...
def get_features_adv(model, dataloader, pretrained_G, max_images=10 ** 10, verbose=False):
    features, labels = [], []
    total = 0
...
#         adv_img = torch.clamp(adv_img, 0, 1)
        ...
        features += list(model(adv_img).data.cpu().numpy())
        labels += list(label.data.cpu().numpy())
...
def get_features_adv_prob(model, dataloader, pretrained_G, prob = 50, clamp =0.1,  max_images=10 ** 10, verbose=False, ds='cifar10'):
    import random
    features, labels = [], []
    total = 0
    print(f'features: {len(dataloader)*256}')
    model.eval()
    imgc = 0
    ad_index = []
    li = 0
#     k2 = 1
    data = []
    labels = []
    for batch_idx, (inputs1, targets, index2) in enumerate(dataloader):
        img = inputs1
        label = targets
        index = batch_idx
        ...
        if k2 <= 50:
            ...
            adv_img = perturbation + img
            k = list(model(adv_img).data.cpu().numpy())
            features += k
            
#             sam = [j for j in range(len(features), len(features)+len(k))]
            if ds == 'cifar10': 
                ind = range(index*256, min((index+1)*256, 10000))
            else:
                ind = range(index*256, min((index+1)*256, 8000))
                
            
            ad_index += ind
            
            data += list(adv_img.detach().cpu().numpy())
            labels += label
        else:
            k = list(model(img).data.cpu().numpy())
            features += k
            labels += list(label.data.cpu().numpy())
            data += list(img.detach().cpu().numpy())
        labels += label
#         li += len(k)
    ...
    dataset = MyDataset(data, labels, transform=transform.Transforms(size=224, s=0.5))
    dataloader = DataLoader(dataset, batch_size=256)
    
    return np.array(features), np.array(labels), imgc, ad_index, dataloader
...
def get_outliers(xin, xood, percentile=90):
    l=[]
    for k in xood:
        if k < np.percentile(xin, percentile):
            l.append(0)
        else:
            l.append(1)
    return l
...
def get_scores_one_cluster(ftrain, ftest, food, shrunkcov=False):
    ...
    print(f'get scores ------- {dood}')
    print(f'get scores sha ------- {dood.shape}')
    print(f'get scores val s------- {dood[0]}')
    return dtest, dood
