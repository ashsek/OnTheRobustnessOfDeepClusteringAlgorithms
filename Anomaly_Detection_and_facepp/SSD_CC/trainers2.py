...
import time
from utils2 import AverageMeter, ProgressMeter, accuracy
...
def supervised(
    ...
    for batch_idx, (inputs1, targets, indexes) in enumerate(train_loader):
        images = inputs1
        target = targets
...
#         bsz = target.shape[0]
    for i, (inputs1, targets, indexes) in enumerate(dataloader):
        images = inputs1
        target = targets
        bsz = target.shape[0]
